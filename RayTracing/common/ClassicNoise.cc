...
static void init(void)
{
	...
	for (i = 0; i < B; i++) {
		...
		for (j = 0; j < 2; j++)
			g2[i][j] = (float)((random() % (B + B)) - B) / B; // two-dimension gradient [-1.0, +1.0] 
		normalize2(g2[i]);

		for (j = 0; j < 3; j++)
			g3[i][j] = (float)((random() % (B + B)) - B) / B; // three-dimension gradient [-1.0, +1.0] 
		normalize3(g3[i]);
	}
...
float ClassicNoise::noise(float x)
{
	return noise1(x);
}

float ClassicNoise::noise(float x, float y)
{
	float v[2] = { x, y };
	return noise2(v);
}
...
float ClassicNoise::fractal(size_t octaves, float x) const
{
	float output = 0.f;
	float denom = 0.f;
...
float ClassicNoise::fractal(size_t octaves, float x, float y) const
{
	float output = 0.f;
	float denom = 0.f;
	float frequency = mFrequency;
...
float ClassicNoise::fractal(size_t octaves, float x, float y, float z) const
{
	float output = 0.f;
	float denom = 0.f;
	float frequency = mFrequency;
